namespace OpenKh.Kh2
{
	public partial class Ps2
	{
		...
		public static byte[] Encode32(byte[] bin, uint bw, uint bh)
		{
			var buffer = new byte[bin.Length];
			for (int i = 0; i < (0x20 * bh); i += 0x20)
			{
				for (uint j = 0; j < (0x40 * bw); j += 0x40)
				{
					long num3 = 0x2000 * ((j / 0x40) + (bw * (i / 0x20)));
					for (int k = 0; k < 0x20; k += 8)
					{
						for (int m = 0; m < 0x40; m += 8)
						{
							...
							for (int n = 0; n < 4; n++)
							{
								...
								for (int num9 = 0; num9 < 0x10; num9++)
								{
									long num10 = (j + m) + (num9 % 8);
                                    long num11 = ((i + k) + (2 * n)) + (num9 / 8);
                                    long index = 4 * (num10 + ((0x40 * bw) * num11));
                                    long num13 = (((4 * tbl32pao[num9]) + num8) + num6) + num3;
