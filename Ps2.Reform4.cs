namespace OpenKh.Kh2
{
	public partial class Ps2
	{
		...
		public static byte[] Encode4(byte[] bin, uint bw, uint bh)
		{
			var buffer = new byte[bin.Length];
			for (int i = 0; i < (0x80 * bh); i += 0x80)
			{
				for (int j = 0; j < (0x80 * bw); j += 0x80)
				{
					long num3 = 0x2000 * ((j / 0x80) + (bw * (i / 0x80)));
					for (int k = 0; k < 0x80; k += 0x10)
					{
						for (int m = 0; m < 0x80; m += 0x20)
						{
							...
							for (int n = 0; n < 4; n++)
							{
								...
								for (int num9 = 0; num9 < 0x80; num9++)
								{
                                    long num10 = (j + m) + (num9 % 0x20);
                                    long num11 = ((i + k) + (4 * n)) + (num9 / 0x20);
                                    long num12 = num10 + ((0x80 * bw) * num11);
                                    ...
                                    long num14 = numArray[num9] / 8;
                                    long num15 = numArray[num9] % 8;
                                    long index = ((num3 + num6) + num8) + num14;
