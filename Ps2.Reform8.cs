namespace OpenKh.Kh2
{
	public partial class Ps2
	{
		...
		public static byte[] Encode8(byte[] bin, uint bw, uint bh)
		{
			var buffer = new byte[bin.Length];
			for (int i = 0; i < (0x40 * bh); i += 0x40)
			{
				for (int j = 0; j < (0x80 * bw); j += 0x80)
				{
					long num3 = 0x2000 * ((j / 0x80) + (bw * (i / 0x40)));
					for (int k = 0; k < 0x40; k += 0x10)
					{
						for (int m = 0; m < 0x80; m += 0x10)
						{
							...
							for (int n = 0; n < 4; n++)
							{
								...
								for (int num9 = 0; num9 < 0x40; num9++)
								{
									long index = ((num3 + num6) + num8) + buffer2[num9];
                                    long num11 = (j + m) + (num9 % 0x10);
                                    long num12 = ((i + k) + (4 * n)) + (num9 / 0x10);
                                    long num13 = num11 + ((0x80 * bw) * num12);
